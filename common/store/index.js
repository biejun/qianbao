...
// 不同应用使用不同的key
let lifeKey = 'lifeData_qb';
let lifeData = {};

try {
	// 尝试获取本地是否存在lifeData变量，第一次启动APP时是不存在的
	lifeData = uni.getStorageSync(lifeKey);
} catch (e) {}

// 需要永久存储，且下次APP启动需要取出的，在state中的变量名
let saveStateKeys = ['vuex_user', 'vuex_token', 'vuex_from', 'vuex_hasLogin', 'vue_phone','vuex_img', 'vuex_lang'];
...
const saveLifeData = function(key, value) {
	...
	if (saveStateKeys.indexOf(key) !== -1) {
		...
		let tmp = uni.getStorageSync(lifeKey);
		// 第一次打开APP，不存在lifeData变量，故放一个{}空对象
		tmp = tmp ? tmp : {};
		tmp[key] = value;
		// 执行这一步后，所有需要存储的变量，都挂载在本地的lifeData对象中
		uni.setStorageSync(lifeKey, tmp);
	}
}
